...
@Component
public class ShipmentTypeUtilChart {
	
	public void generatePieChart(String location,List<Object[]> list) {
		
		// a. Create Data Set and read List<Object[]> into Dataset Value
		DefaultPieDataset dataset = new DefaultPieDataset();
		for(Object[] ob:list) {
			dataset.setValue(ob[0].toString(),Double.valueOf((ob[1].toString())));
		}
		
		// b.Convert Data data into JFreeChart object using ChartFactory class
		JFreeChart chart = ChartFactory.createPieChart3D("SHIPMENT TYPE MODE", dataset);
		
		// c.Convert JFree chart object to one Image using Chart Util class
		
		try {
			ChartUtils.saveChartAsJPEG(new File(location+"/Product.jpg"), chart, 400, 300);
		} catch (IOException e) {
			e.printStackTrace();
		}
     ...
     public void generateBarChart(String location,List<Object[]> list) {
    	...
    	// a. Create DataSet and read List<Object[]> into DataSet values
 		...
 		try {
 			ChartUtils.saveChartAsJPEG(new File(location+"/Product.jpg"), chart, 500, 350);
 		} catch (IOException e) {
 			e.printStackTrace();
 		}
 	}
